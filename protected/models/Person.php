 ...
 * @property TblDepartment $idDepartment
 */
class Person extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, id_department', 'required'),
			array('id_department', 'numerical', 'integerOnly'=>true),
			array('name, phone, email', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, phone, email, id_department', 'safe', 'on'=>'search'),
		);
	}
	...
	public static function model($className=__CLASS__)
	{
        ...
        {
            $models = Person::model()->findAll(array('order' => 'name'));
